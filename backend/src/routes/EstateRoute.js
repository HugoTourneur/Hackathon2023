import {getFirestore, addDoc, getDoc, getDocs, updateDoc, deleteDoc, collection, doc, where, query} from "firebase/firestore"
...
const EstateRoute = ({app, firebase}) => {
    ...
    app.post("/api/estates", async (req, res) => {
        const { title, location, value, type, max_shares, status, image } = req.body
        try {
            const docRef = await addDoc(collection(db, 'estates'), { title, location, value, type, max_shares, status, image })
    ...
    app.put("/api/estates/:id", async (req, res) => {
        ...
        const { id } = req.params
        try {
            await updateDoc(doc(db, 'estates', id), { title, location, value, type, max_shares, status, image })
            res.json({id})
        } catch (error) {
            console.log(error)
            res.status(400).json({message: "An error occurred"})
    ...
    app.post("/api/estates/:id/buy-parts", isLogged, async (req, res) => {
        ...
        try {
            ...
            const docRef = await addDoc(collection(db, 'parts'), { parts, user_id, estate_id: id })
